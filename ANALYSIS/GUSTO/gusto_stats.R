...
pacman::p_load(tidyverse, dplyr, plyr, Rmisc, BayesFactor) 
...
pp <- ggplot(HED.means, aes(x = cond, y = perceived_liking, 
  ...
  geom_crossbar(data = dfH, aes(y = perceived_liking, ymin=perceived_liking-ci, ymax=perceived_liking+ci,), width = 0.2 , alpha = 0.1)+
...
pp <- ggplot(df, aes(x = cond, y = betas, 
  ...
  geom_flat_violin(scale = "count", trim = FALSE, alpha = .2, aes(fill = condition, color = NA))+
  geom_point(aes(x = condjit), alpha = .3,) +
  geom_crossbar(data = dfH, aes(y = betas, ymin=betas-ci, ymax=betas+ci), width = 0.2 , alpha = 0.1)+
