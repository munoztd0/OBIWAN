...
for i = 1:length(param.task)
    ...
    param.Cnam{i} = {'start',... %1
     ...
     param.onset{i} = {'ONS.onsets.trialstart',... %1
        'ONS.onsets.odor.run1', ...
        'ONS.onsets.odor.run2', ...
        'ONS.onsets.odor.run3', ...
        'ONS.onsets.liking',...%5 
    ...
    param.duration{i} = {'ONS.durations.trialstart',...
        'ONS.durations.odor.run1',...
        'ONS.durations.odor.run2',...
        'ONS.durations.odor.run3',...
    ...
    param.modul{i} = {'none',...%1
        'ONS.modulators.odor.run1',... %2
        'ONS.modulators.odor.run2',... %2
        'ONS.modulators.odor.run3',... %2
        'none',... %3
        'none'}; %6
...
%% function section
    ...
    function [] = doContrasts(subjoutdir, param, SPM)
        ...
        %con1
        Ctnames{1} = 'odor_run1_lik';
        weightPos  = ismember(conditionName, {'task-hed.odor.run1xlik^1'}) * 1; 
        Ct(1,:)    = weightPos;
        
        %con2
        Ctnames{2} = 'odor_run2_lik';
        weightPos  = ismember(conditionName, {'task-hed.odor.run2xlik^1'}) * 1; 
        Ct(2,:)    = weightPos;
        
        %con3
        Ctnames{3} = 'odor_run3_lik';
        weightPos  = ismember(conditionName, {'task-hed.odor.run3xlik^1'}) * 1; 
        Ct(3,:)    = weightPos;
        
        
        % con4
        Ctnames{4} = 'odor-run1_int';
        weightPos  = ismember(conditionName, {'task-hed.odor.run1xint^1'}) * 1; 
        Ct(4,:)    = weightPos;
        
        % con5
        Ctnames{5} = 'odor-run2_int';
        weightPos  = ismember(conditionName, {'task-hed.odor.run2xint^1'}) * 1; 
        Ct(5,:)    = weightPos;
        
        
        % con6
        Ctnames{6} = 'odor-run3_int';
        weightPos  = ismember(conditionName, {'task-hed.odor.run3xint^1'}) * 1; 
        Ct(6,:)    = weightPos;
        
        
        %con7
        Ctnames{7} = 'odor_run1_2_lik';
        weightPos  = ismember(conditionName,  {'task-hed.odor.run1xlik^1', 'task-hed.odor.run2xlik^1'}) * 0.5;
        Ct(7,:)    = weightPos;
        
        %con8
        Ctnames{8} = 'odor_run1_2_int';
        weightPos  = ismember(conditionName,  {'task-hed.odor.run1xint^1', 'task-hed.odor.run2xint^1'}) * 0.5;
        Ct(8,:)    = weightPos;
        
        

        % con9
        Ctnames{9} = 'run1-run2+3_lik';
        weightPos  = ismember(conditionName, {'task-hed.odor.run1xlik^1'}) * 2; 
        weightNeg  = ismember(conditionName, {'task-hed.odor.run2xlik^1', 'task-hed.odor.run3xlik^1'}) * -1;
        Ct(9,:)    = weightPos+weightNeg;
        
        
        % con9
        Ctnames{10} = 'run1-run2+3_int';
        weightPos  = ismember(conditionName, {'task-hed.odor.run1xint^1'}) * 2; 
        weightNeg  = ismember(conditionName, {'task-hed.odor.run2xint^1', 'task-hed.odor.run3xint^1'}) * -1;
