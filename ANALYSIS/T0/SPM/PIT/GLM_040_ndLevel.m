...
if do_covariate % covariate of interest name become folder
   ...
   covariateNames = {'CSp-CSm_force_meancent'}; %9
   contrastNames = {'CSp_CSm'};%2
   conImages = {'con-0001'};
    ...
    for c = 1:length(covariateNames)
        ...
        for n = 1:length(contrastNames)
            ...
            for s = 1:length(cov.ID)
                ...
                idX = [ '*' num2str(cov.IDX) '*.nii' ];
                conAll{s}  = dir(idX);
            end
            
            for j =1:length(conAll)
                matlabbatch{1}.spm.stats.factorial_design.des.t1.scans{j,1} = [groupdir conAll{j}.name ',1'];
