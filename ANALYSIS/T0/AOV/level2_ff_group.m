function matlabbatch = level2_ff_group(condirpat, factors, aovdir, conImages, varargin)

dbstop if error

%% define path

%covariates(1).name = 'Liking';
%covariates(1).values = [1...n]

def =   { ...
    'covariates',   [],                     ...
    'conpat',       'con*nii',              ...
    'conweights',   'auto',                 ...
    'implicitmask',  1,                     ... %changed here
    'mask',         '',                     ...
    'nan2zero',      1,                     ...
    'negativecon',   0,                     ...
    'outdir',       [aovdir],                     ...
...
if nargin<2, fprintf('\t= DEFAULT SETTINGS =\n'); disp(vals); return; end
fprintf('\n\t= CURRENT SETTINGS =\n'); disp(vals);
...
for n = 1:length(conImages)
    ...
    for i = 1:length(conidx)
        conGroupX = conidx{i};
        selectcon{:, i}  = spm_select('List',condirpat,['^'  '.*' conGroupX '.*' conImages{n} '.nii']); % select constrasts 
         %spm_select('List',groupdir,['^'  '.*' conImageX '.nii']); % select const
    end
    %conAll = reshape(permute(selectcon,[2,1,3]),size(selectcon,2),[])';
    conAll = vertcat(cellstr(selectcon{1, 1}), cellstr(selectcon{1, 2}));
