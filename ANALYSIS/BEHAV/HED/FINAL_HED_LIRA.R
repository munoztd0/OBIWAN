...
pacman::p_load(tidyverse, ggplot2, dplyr, lmerTest, afex, r2glmm, optimx, sjPlot, emmeans)
...
AGE = ddply(HED,~session,summarise,mean=mean(age),sd=sd(age))
BMI = ddply(HED,~session,summarise,mean=mean(BMI_t1),sd=sd(BMI_t1))
...
#FOR MODEL SELECTION we followed Barr et al. (2013) approach SEE --> CODE/ANALYSIS/BEHAV/HED/MODEL_SELECTION.R
...
model = mixed(likZ ~ condition*time*intervention + gender + ageZ + diff_bmiZ + (time*condition|id) , 
              data = HED, method = "PB", control = control, REML = FALSE, args.test = list(nsim = 100))
...
R2 = r2beta(model,method="nsj") #R(m)2, the proportion of variance explained by the fixed predictors 


# MODEL ASSUMPTION CHECKS : REML = TRUE -----------------------------------
mod = update(model, REML = TRUE) 

#explicitly check correlation (between individuals’ intercept and slope residuals)
VarCorr(random.slope.model) #The correlation between the random intercept and slopes is pretty high, so we keep them
...
plot_model(mod, type = "diag") #super cool sjPlots for checking assumptions -> not bad
...
# THE END - Thanks to Ben Meulmann and Yoann Stussi -----------------------------------------------------------------
