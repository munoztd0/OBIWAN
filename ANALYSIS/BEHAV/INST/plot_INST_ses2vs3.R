...
#subset
OBIWAN_INST  <- subset(OBIWAN_INST_full, group == 'obese') #only session 2
#OBIWAN_INST_control  <- subset(OBIWAN_INST, session == 'control') 
#OBIWAN_INST_obese  <- subset(OBIWAN_INST, session == 'obese') 
OBIWAN_INST_sec  <- subset(OBIWAN_INST, session == 'second') #only session 2
OBIWAN_INST_third  <- subset(OBIWAN_INST, session == 'third') #only session 3
...
btg = ddply(OBIWAN_INST, .(session, trial), summarise,  grips = mean(grips, na.rm = TRUE) ) 

# get means by condition and trial
#btc = ddply(OBIWAN_INST, .(condition, trial), summarise,  grips = mean(grips, na.rm = TRUE) ) 
#btcg = ddply(OBIWAN_INST, .(session, condition, trial), summarise,  grips = mean(grips, na.rm = TRUE)  ) 
...
bsTg = ddply(OBIWAN_INST, .(id, session, trial), summarise, grips = mean(grips, na.rm = TRUE)  ) 
#bsCg= ddply(OBIWAN_INST, .(id, session, condition), summarise, grips = mean(grips, na.rm = TRUE) ) 
#bsTCg = ddply(OBIWAN_INST, .(id, session, trial, condition), summarise, grips = mean(grips, na.rm = TRUE)  ) 

tot_n = length(unique(OBIWAN_INST$id))
tot_sec = length(unique(OBIWAN_INST_sec$id))
#tot_obese = length(unique(OBIWAN_INST_third$id))
tot_third = length(unique(OBIWAN_INST_third$id))
...
plt2 <- ggplot(df_INST, aes(x = trial, y = grips, fill = session, color=session)) +
  ...
  scale_y_continuous(expand = c(0, 0),  limits = c(6,18),  breaks=c(seq.int(6,18, by = 2))) +
  ...
  labs(x = "Trial",
       y = "Number of Grips",
       caption = "Second session (n=63)  Vs Third session (n=50) \nError bars represent SEM for within-subject design using method from Morey (2008)")



pdf(file.path(figures_path,paste(task, 'time_ses2vs3.pdf',  sep = "_")),
    width     = 7.5,
    height    = 6)
