...
cont = emmeans(mod, pairwise~ condition|BMI_t1, at = list(BMI_t1 = c(22.24,32.65,37.72)), side = ">", adjust = "tukey") #those are the mean per group intercepts
...
pl <-  ggplot(df.predicted, aes(x = BMI_t1, y = estimate)) +
      geom_line( color = 'royalblue', size = 1) +
       geom_point(data = df.cont, size = 1, color = 'blue')+
      geom_ribbon(aes(ymin=lower.CL, ymax=upper.CL), alpha = .5, fill = 'lightgrey') +
...
plt = pl + 
  ...
  labs(title = "PIT Effect by BMI", 
       ...
       caption = "Error range represent 95% CI for the model estimated prediction\n
       Main effect of Pavlovian cue (CS+ > CS-), p = 0.012\n
       Two-way interaction (BMIxCondition), p = 0.086\n
       Post-hoc test at fixed BMI class, HW: p = 0.51, OB class I: p = 0.0062, OB class II: p = 0.0029\n 
