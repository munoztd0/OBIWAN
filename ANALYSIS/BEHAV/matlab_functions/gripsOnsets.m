function all_gripsOnsets =  gripsOnsets(threshold,nlines,ncolons,force,trialOnset,reward1,reward2,duration)



% This function compute the time of each grips based on the fact that each
% vector is calibrated for duration plus possibles pauses of variable duration
...
for l = 1:ncolons
    ...
    for i = 1:nlines - 2
        ...
        x = force (:,l);
        sr = x;
        sr(isnan(sr)) = []; % finding sampling rate
        srate = size(sr,1);
        ...
        if x(i) < threshold && x (i+1) > threshold
            ...
            grips = grips + 1;
            gripsOnsets (grips) = trialOnset(l) + (i*((duration(l)/srate)));%+ rewardtime; % the onset of the grips is equal to the onset of the trial + the time that has passed (the vector has been calibrated to record values for 12 s)
