...
dflist <- lapply(mget(tables),function(x)subset(x, session == 'third'))
...
write.table(lik_c, (file.path(analysis_path, "HED_covariateT1_Ratings.tsv")), row.names = F, sep="\t")
write.table(ms_c, (file.path(analysis_path, "HED_covariateT1_REWARD.tsv")), row.names = F, sep="\t")
write.table(emp_c, (file.path(analysis_path, "HED_covariateT1_CONTROL.tsv")), row.names = F, sep="\t")
write.table(dtf_c, (file.path(analysis_path, "HED_covariateT1.tsv")), row.names = F, sep="\t")
...
write.table(eff_c, (file.path(analysis_path, "PIT_covariateT1_Force.tsv")), row.names = F, sep="\t")
write.table(dtf_c, (file.path(analysis_path, "PIT_covariateT1.tsv")), row.names = F, sep="\t")
