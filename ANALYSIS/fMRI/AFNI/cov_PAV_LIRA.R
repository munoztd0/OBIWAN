...
PAV = PAV %>% filter(id %notin% c(210, 214, 216, 219, 222, 223, 230, 233, 240, 242, 245, 247, 249, 256, 258, 263, 266, 267)) #& 266??

bs = ddply(PAV, .(id, condition, time, intervention, gender), summarise, RT = mean(RT, na.rm = TRUE))
bs$RT = scale(bs$RT)
...
CSp_RT_t0 = subset(bs, time == '0' & condition == 1)
CSp_RT_t1 = subset(bs, time == '1'& condition == 1)
CSm_RT_t0 = subset(bs, time == '0' & condition == -1)
CSm_RT_t1 = subset(bs, time == '1'& condition == -1)


path <-'~/OBIWAN/DERIVATIVES/GLM/SPM/pav/GLM-01_0'
write.table(CSp_RT_t0, (file.path(path, "CSp_RT.txt")), row.names = F, sep="\t")
write.table(CSm_RT_t0, (file.path(path, "CSm_RT.txt")), row.names = F, sep="\t")
path <-'~/OBIWAN/DERIVATIVES/GLM/SPM/pav/GLM-01_1'
write.table(CSp_RT_t1, (file.path(path, "CSp_RT.txt")), row.names = F, sep="\t")
write.table(CSm_RT_t1, (file.path(path, "CSm_RT.txt")), row.names = F, sep="\t")
