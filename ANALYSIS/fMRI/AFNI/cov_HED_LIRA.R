...
bs = ddply(PIT, .(id, condition, time, intervention, gender), summarise, eff = mean(gripC, na.rm = TRUE))
...
diff = subset(bs, condition == "1")
diff$eff = diff$eff - CSm$eff
eff = diff %>% select(id, eff, time, intervention)

t0 = subset(eff, time == '0')
t1 = subset(eff, time =='1')

# EFFORT ------------------------------------------------------------------
CSp_CSm_eff_t0 = eff
CSp_CSm_eff_t1 = eff

CSp_CSm_eff_t0$eff[CSp_CSm_eff_t0$time == '0'] <- 0
CSp_CSm_eff_t1$eff[CSp_CSm_eff_t1$time == '1'] <- 0

#write.table(CSp_CSm_eff_t0, (file.path(path, "CSp_CSm_eff_ob.txt")), row.names = F, sep="\t")
#write.table(CSp_CSm_eff_t1, (file.path(path, "CSp_CSm_eff_hw.txt")), row.names = F, sep="\t")
