...
mdldir        = fullfile (homedir, '/DERIVATIVES/FSL');
SPMdir        = fullfile (homedir, '/DERIVATIVES/GLM');
...
for j = 1:length(task)
    ...
    for  i=1:length(subj)
        ...
        subjoutdir = fullfile(SPMdir, char(task), ana_name,  subjX,'output');
        if exist(subjoutdir, 'dir')
            cd(subjoutdir)
            copyfile('mask.nii', subjdir)
    end
