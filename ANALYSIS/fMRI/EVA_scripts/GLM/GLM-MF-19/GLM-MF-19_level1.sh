...
echo "preparing GLM-MF-19 for subject ${subjectID} session ${runID}"
...
outDir=/home/eva/PAVMOD/DATA/brain/MODELS/FSL/GLM/GLM-MF-19/
# directory containing the timing input
timingDir=/home/eva/PAVMOD/DATA/brain/MODELS/SPM/GLM-MF-19/sub-${subjectID}/timing/
...
mkdir ${outDir}
outDir=/home/eva/PAVMOD/DATA/brain/MODELS/FSL/GLM/GLM-MF-19/sub-${subjectID}/run-${runID}/
mkdir ${outDir}
...
if [ ${runID} = 03 ]; then
  ...
  if [ ${subjectID} = 10 ] || [ ${subjectID} = 18 ] ; then # list of subject with no action for this runYYY
    cp ${codeDir}level1_template-GLM-MF-19-E-noaction.fsf $level1Template
  else
    cp ${codeDir}level1_template-GLM-MF-19-E.fsf $level1Template
  fi
    ...
    if [ ${subjectID} = 06 ]; then
      ...
      cp ${codeDir}level1_template-GLM-MF-19-L-noaction.fsf $level1Template
    else
      cp ${codeDir}level1_template-GLM-MF-19-L.fsf $level1Template
    fi
