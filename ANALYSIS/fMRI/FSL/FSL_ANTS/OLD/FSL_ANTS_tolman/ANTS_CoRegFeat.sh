...
# directory with distorted images
rawDir=/home/evapool/PAVMOD/DATA/brain/ICA_ANTS/${sbj}/RUN_${ses}.ica/
# directory with ICA and FEAT preproc data
icaDir=/home/evapool/PAVMOD/DATA/brain/tmp/${sbj}/RUN_${ses}.feat/
...
rawFuncImage=${rawDir}filtered_func_data_clean
funcImage=${icaDir}filtered_func_data

# extract a sample to speed up processing, and mean function to  bias correct
echo "Extracting single volume and mean volume to speed processing and do bias correction $(date +"%T")"

fslroi ${funcImage} ${icaDir}func_sample 0 1

fslmaths ${funcImage} -Tmean ${icaDir}mean_func_clean

/usr/local/ANTs/build/bin/N4BiasFieldCorrection -i ${icaDir}mean_func_clean.nii.gz -o ${icaDir}func_sample_bias.nii.gz --convergence [50x50x30x20,0.0] -d 3 -s 3 -b [300]
...
flirt -in ${icaDir}func_sample_bias -ref ${moving_T2} -dof 7 -out ${icaDir}tmp_Func_to_T2 -omat ${icaDir}tmp_Func_to_T2.mat
...
# go from functional -> anatomical as the warp for multi-volume transform

echo "Apply series of transformations all the way from func to lowres atlas (in MNI space: test with 10 images only)"
fslroi ${icaDir}filtered_func_data.nii.gz ${icaDir}func_firstTenTest 0 10

# FOR TESTING (we use only the first 10 images)
