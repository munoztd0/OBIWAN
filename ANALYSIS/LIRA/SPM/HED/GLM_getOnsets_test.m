...
for s = 1:length(session)
    ...
    for  i=1:length(subj)
        ...
        VV = nan(length(CONDITIONS),1); %taste value vector

        Vmilk   =  1;
        Vneutral = -1;

        % recode taste in values
        for t = 1:length(CONDITIONS)
            taste         = CONDITIONS{t};
            if strcmp ('MilkShake',  taste)
                VV(t) = Vmilk;    
            elseif strcmp ('Empty',  taste)
                VV(t) = Vneutral;         
            end
        end
        ...
        modulators.taste.reward.chAbs  = ChAbs (strcmp ('MilkShake', CONDITIONS));
        modulators.taste.neutral.chAbs = ChAbs (strcmp ('Empty', CONDITIONS));

        modulators.taste.conc.chAbs= vertcat(modulators.taste.reward.chAbs, modulators.taste.neutral.chAbs);

        %mean_centering mod
        cent_chAbs = mean(modulators.taste.conc.chAbs);

        for j = 1:length(modulators.taste.conc.chAbs)
            modulators.taste.conc.chAbs(j)  = modulators.taste.conc.chAbs(j) - cent_chAbs;
        end

        modulators.taste.conc.chAbs = modulators.taste.conc.chAbs(Idx,:);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
        %mod for improvement change
        modulators.taste.reward.ch  = Ch (strcmp ('MilkShake', CONDITIONS));
        modulators.taste.neutral.ch= Ch (strcmp ('Empty', CONDITIONS));

        modulators.taste.conc.ch= vertcat(modulators.taste.reward.ch, modulators.taste.neutral.ch);

        %mean_centering mod
        cent_ch = mean(modulators.taste.conc.ch);

        for j = 1:length(modulators.taste.conc.ch)
            modulators.taste.conc.ch(j)  = modulators.taste.conc.ch(j) - cent_ch;
        end

        modulators.taste.conc.ch = modulators.taste.conc.ch(Idx,:);
        ...
        for ii = 1:length(name)
            ...
            if strcmp (nameX, 'taste')  % for structure that contains substuctures
                ...
                subsubstr = {'lik'; 'int'; 'fam'; 'ch'; 'chAbs'}; % specify the subsubstructures names 
