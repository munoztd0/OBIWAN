...
name_ana = 'GLM-14-control'; % output folder for this analysis
...
%% function section
    function [SPM] = doFirstLevel(subjoutdir,subjfuncdir,subjanatdir, name_ana, param, subjX)
        ...
        for ses = 1:nruns
            ...
            targetscan         = dir (fullfile(smoothfolder, ['*' runX '*smoothBold*' param.im_format]));
        ...
        for ses = 1:nruns
            ...
            ONSname = spm_select('List',[subjoutdir '/timing/'],[name_ana '_task-' runX '_onsets.mat']);
            ...
            for cc=1:nconds
                ...
                if ~ isempty(eval(param.Cnam{ses}{cc})) % only if the onsets are not all 0
                    ...
                    SPM.Sess(ses).U(c).orth      = 1; % orthogonalization
%                    on
...
%                         SPM.Sess(ses).C.name = rnam;
        ...
        %--------------------------------------------------------------------------
        SPM.xM.VM  = spm_vol(char({[subjanatdir '/sub-' subjX '_ses-first_acq-ANTsNorm_T2w.nii']})); % here enter the mask based on the subject anatomical
    ...
    function [] = doContrasts(subjoutdir, param, SPM)
        ...
        % con3
        Ctnames{3} = 'intensity';
        weightPos  = ismember(conditionName, {'run1.swallowxintensity^1'}) * 1;
        Ct(3,:)    = weightPos;
        
        % con4 
        Ctnames{4} = 'familiarity';
        weightPos  = ismember(conditionName, {'run1.swallowxfamiliarity^1'}) * 1;
        Ct(4,:)    = weightPos;
