...
data$V5 = c(202, 203, 204, 209, 213, 217, 220, 224, 225, 235, 236, 237, 238, 239, 241, 246, 250, 259, 264, 265, 266, 269, 270, 205, 206, 207, 211, 215, 218, 221, 227, 229, 230, 231, 232, 244, 248, 251, 252, 253, 254, 262, 268)
#`%notin%` <- Negate(`%in%`); data = filter(data, V5 %notin% c(239)) #outlier doesnt change results because always high in both sessions

colnames(data) <- c('Milk_pre', 'Milk_post', 'Tasteless_pre', 'Tasteless_post', 'id')

df1 <- gather(data, session, milk, c(Milk_pre,Milk_post), factor_key=TRUE)
df1$session = as.factor(revalue(df1$session, c(Milk_pre="pre", Milk_post="post"))); df1 = select(df1, c(id, session, milk))
                        
df2 <- gather(data, session, tasteless, c(Tasteless_pre,Tasteless_post), factor_key=TRUE)
df2$session = as.factor(revalue(df2$session, c(Tasteless_pre="pre", Tasteless_post="post"))); df2 = select(df2, c(id, session, tasteless)); diff = df2; diff$score =  df2$tasteless - df1$milk
# df = merge(df1, df2 , by=c('id', 'session')); df <- gather(df, condition, beta_coef, c(milk,tasteless), factor_key=TRUE)

info <- read.delim(file.path(data_path,'info_expe.txt'), header = T, sep ='') # 

df = merge(diff, info , by= 'id');
...
pp <- ggplot(df, aes(x = cond, y = score, 
  ...
  labs(y='mOFC Betas (a.u.)' , x = '',
  title='Milkshake > Tastless \n(modulated by liking ratings)') +
