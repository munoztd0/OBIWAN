function GLM_01_stLevel()

% intended for OBIWAN VBM
...
for i = 1:length(subj)
        ...
        batch{1}.spm.spatial.smooth.data = {[sourcefiles subjX '/ses-first/anat/mwc1' subjX '_ses-first_acq-ANTsNorm_T1w.nii,1']
        ...
        batch{1}.spm.spatial.smooth.fwhm = [8 8 8];
        batch{1}.spm.spatial.smooth.dtype = 0;
        batch{1}.spm.spatial.smooth.im = 0;
        batch{1}.spm.spatial.smooth.prefix = 's';
    ...
    if copytissues == 1
        ...
        for y =1:size(Timages,1)
            copyfile(['smw' (Timages(y,:)) subjX '_ses-first_acq-ANTsNorm_T1w.nii'],[groupdir, subjX '_' (Fimages(y,:)) '.nii'])
        end
