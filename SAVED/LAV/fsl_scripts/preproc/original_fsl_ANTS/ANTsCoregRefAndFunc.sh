...
subAnatDir=/home/OBIWAN/DATA/STUDY/DERIVED/ANTS/sub-${sbj}/ses-first/anat/
# Directory with run-specific files
#runDir=/home/OBIWAN/DATA/STUDY/DERIVED/ICA_ANTS/sub-${sbj}/ses-${ses}/func/
runDir=/home/OBIWAN/DATA/STUDY/DERIVED/ANTS/sub-${sbj}/ses-${ses}/func/
...
rawFuncImage=${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain
#funcImage=${icaDir}filtered_func_data_clean_unwarped
funcImage=${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain_unwarped
...
fslroi ${funcImage} ${runDir}task-${task}_func_sample 0 1
# we need the mean scan to realign functional within session
#fslmaths ${funcImage} -Tmean ${icaDir}mean_func_clean
fslmaths ${funcImage} -Tmean ${runDir}task-${task}_mean_func_clean
...
/usr/local/ants/bin/N4BiasFieldCorrection -i ${runDir}task-${task}_mean_func_clean.nii.gz -o ${runDir}task-${task}_func_mean_bias.nii.gz --convergence [100x100x100x100,0.0] -d 3 -s 3 -b [300]
...
bet ${runDir}task-${task}_func_mean_bias.nii.gz ${runDir}task-${task}_func_mean_bias.nii.gz -f 0.25 -R
...
flirt -in ${runDir}task-${task}_func_mean_bias -ref ${moving_T2} -dof 6 -cost mutualinfo -out ${runDir}task-${task}_tmp_Func_to_T2 -omat ${runDir}task-${task}_tmp_Func_to_T2.mat
...
flirt -in ${runDir}task-${task}_func_mean_bias -ref ${moving_T2} -dof 12 -cost mutualinfo -init ${runDir}task-${task}_tmp_Func_to_T2.mat -nosearch -out ${runDir}task-${task}_tmp_Func_to_T2_improved -omat ${runDir}task-${task}_tmp_Func_to_T2.mat
...
$c3d_affine_tool -src ${runDir}task-${task}_func_mean_bias -ref ${moving_T2} ${runDir}task-${task}_tmp_Func_to_T2.mat -fsl2ras -oitk ${runDir}task-${task}_itk_transform_Func_To_T2.txt
...
${warpTool} 3 ${runDir}task-${task}_func_sample.nii.gz ${runDir}task-${task}_func_sample_ANTsFuncT2.nii.gz \
        -R ${fixed_T2lowres}.nii.gz \
        ${outPrefix}_xfm1Warp.nii.gz ${outPrefix}_xfm0GenericAffine.mat \
        ${runDir}task-${task}_itk_transform_Func_To_T2.txt
...
fslroi ${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain_unwarped.nii.gz ${runDir}task-${task}_func_firstTenTest 0 10
...
/usr/local/ants/bin/WarpTimeSeriesImageMultiTransform 4 ${runDir}task-${task}_func_firstTenTest.nii.gz ${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain_unwarped_Coreg.nii.gz \
	   -R ${fixed_T2lowres}.nii.gz \
      ${outPrefix}_xfm1Warp.nii.gz ${outPrefix}_xfm0GenericAffine.mat \
	   ${runDir}task-${task}_itk_transform_Func_To_T2.txt
...
/usr/local/ants/bin/WarpTimeSeriesImageMultiTransform 4 ${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain_unwarped.nii.gz ${runDir}sub-${sbj}_ses-${ses}_task-${task}_run-01_bold_reoriented_brain_unwarped_Coreg.nii.gz \
	-R ${fixed_T2lowres}.nii.gz \
  ${outPrefix}_xfm1Warp.nii.gz ${outPrefix}_xfm0GenericAffine.mat \
	${runDir}task-${task}_itk_transform_Func_To_T2.txt
...
/usr/local/ants/bin/N4BiasFieldCorrection -i ${runDir}task-${task}_func_sample.nii.gz -o ${runDir}task-${task}_func_sample_bias.nii.gz --convergence [100x100x100x100,0.0] -d 3 -s 3 -b [300]
#flirt -in ${icaDir}func_sample_bias -ref ${fixed_T2lowres}.nii.gz -dof 7 -out ${icaDir}func_sample_bias_alignT2
flirt -in ${runDir}task-${task}_func_sample_bias -ref ${fixed_T2lowres}.nii.gz -dof 7 -out ${runDir}task-${task}_func_sample_bias_alignT2
