...
for j = 1:length(runs)
    ...
    for  i=1:length(subj)
        ...
        onsets.swallow.reward                 = ONSETS.break(strcmp ('MilkShake', CONDITIONS));
        onsets.swallow.control                 = ONSETS.break(strcmp ('Empty', CONDITIONS));        
        
        durations.swallow.reward              = zeros (length(onsets.swallow.reward),1);
        durations.swallow.control              = zeros (length(onsets.swallow.control),1);
        
        modulators.swallow.reward.liking      = BEHAVIOR.liking(strcmp ('MilkShake', CONDITIONS));
        modulators.swallow.reward.intensity   = BEHAVIOR.intensity(strcmp ('MilkShake', CONDITIONS));
        modulators.swallow.reward.familiarity = BEHAVIOR.familiarity(strcmp ('MilkShake', CONDITIONS));
        modulators.swallow.control.liking      = BEHAVIOR.liking(strcmp ('Empty', CONDITIONS));
        modulators.swallow.control.intensity   = BEHAVIOR.intensity(strcmp ('Empty', CONDITIONS));
        modulators.swallow.control.familiarity = BEHAVIOR.familiarity(strcmp ('Empty', CONDITIONS));
        ...
        for ii = 1:length(name)
            ...
            if strcmp (nameX, 'swallow') % for structure that contains modulators
                substr = {'reward'; 'control'}; % specify the substructure names
                ...
                database.(nameXX) = [num2cell(onsets.(nameX)), num2cell(durations.(nameX)), num2cell(ones(length(onsets.(nameX)),1))]; % put one as a modulator
