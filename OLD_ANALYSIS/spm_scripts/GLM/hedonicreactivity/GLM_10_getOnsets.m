...
for j = 1:length(runs)
    ...
    for  i=1:length(subj)
        ...
        onsets.taste                = ONSETS.liquid;
        durations.taste             = DURATIONS.liquid;
        modulators.taste.reward     = strcmp ('MilkShake', CONDITIONS); % set to one

        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Recompute onset of swalling signal
        onsets.swallow          = ONSETS.liquid+DURATIONS.liquid;
        durations.swallow       = zeros (length(onsets.taste),1);
        modulators.swallow      = ones (length(onsets.taste),1);% set to one
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        name = {'trialstart'; 'taste'; 'swallow'; 'liking'; 'intensity'; 'familiarity'; 'rinse'};
        
        for ii = 1:length(name)
            
            nameX = char(name(ii));
            
            if strcmp (nameX, 'taste') % for structure that contains modulators
