...
#		qsub -o ~/ClusterOutput -j oe -l nodes=1,walltime=01:00:00,pmem=5GB -M evapool@caltech.edu -m e -q batch -N warpFuncT2_Sub_${subj}_ses_${sessionID}_task-${taskID} -F "${subj} ${sessionID} ${taskID}" ${funcScript}
		qsub -o /home/OBIWAN/ClusterOutput -j oe -l walltime=1:00:00,pmem=5GB -M lavinia.wuensch@etu.unige.ch -m e -l nodes=1 -q queue1 -N warpFuncT2_Sub_${subj}_ses_${sessionID}_task-${taskID} -F "${subj} ${sessionID} ${taskID}" ${funcScript}
