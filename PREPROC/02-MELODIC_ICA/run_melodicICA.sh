  ...
  subjectID=${group}${subjID}
	...
	do
		...
		# spawn session jobs to the cluster after the subject level work is complete
      	qsub -o /home/OBIWAN/ClusterOutput -j oe -l walltime=25:00:00,pmem=16GB -M david.munoz@etu.unige.ch -m n -l nodes=1 -q queue2 -N ICA-${subjectID}_ses-${sessionID}_task-${taskID} -F "${subjectID} ${sessionID} ${taskID}" ${melodicScript}
